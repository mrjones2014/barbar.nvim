...
function bufferline.setup(options)
  ...
  end, { count = true, desc = 'Go to the next buffer' })

  create_user_command('BufferPrevious', function(tbl)
    api.goto_buffer_relative(-math.max(1, tbl.count))
  end, { count = true, desc = 'Go to the previous buffer' })

  create_user_command('BufferGoto', function(tbl)
    local index = tbl.args
    validate({ index = { index, 'number' } })
    api.goto_buffer(index)
  end, { desc = 'Go to the buffer at the specified index', nargs = 1 })
  ...
  create_user_command('BufferMove', function(tbl)
    command('BufferMovePrevious ' .. tbl.count)
  end, { count = true, desc = 'Synonym for `:BufferMovePrevious`' })

  create_user_command('BufferMoveNext', function(tbl)
    api.move_current_buffer(math.max(1, tbl.count))
  end, { count = true, desc = 'Move the current buffer to the right' })

  create_user_command('BufferMovePrevious', function(tbl)
    api.move_current_buffer(-math.max(1, tbl.count))
  end, { count = true, desc = 'Move the current buffer to the left' })

  create_user_command('BufferPick', api.pick_buffer, { desc = 'Pick a buffer' })

  create_user_command('BufferPin', function()
    api.toggle_pin()
  end, { desc = 'Un/pin a buffer' })
  ...
  create_user_command('BufferOrderByDirectory', api.order_by_directory, { desc = 'Order the bufferline by directory' })
  ...
  create_user_command('BufferDelete', function(tbl)
    bbye.bdelete(tbl.bang, tbl.args, tbl.mods)
  end, { bang = true, complete = 'buffer', desc = 'Synonym for `:BufferClose`', nargs = '?' })

  create_user_command('BufferWipeout', function(tbl)
    bbye.bwipeout(tbl.bang, tbl.args, tbl.mods)
  end, { bang = true, complete = 'buffer', desc = 'Wipe out the buffer', nargs = '?' })
  ...
  create_user_command('BufferScrollLeft', function(tbl)
    render.scroll(-math.max(1, tbl.count))
  end, { count = true, desc = 'Scroll the bufferline left' })

  create_user_command('BufferScrollRight', function(tbl)
    render.scroll(math.max(1, tbl.count))
  end, { count = true, desc = 'Scroll the bufferline right' })
